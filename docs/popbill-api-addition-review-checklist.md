...
| 3 | 홈택스수집 | 매입/매출 세금계산서 확인에 유용하지만 인증/수집 흐름이 복잡함 | 수집 요청, 작업 상태, 결과 목록 |
| 4 | 전자세금계산서 | 기존 문서/매출 기능과 연결 가능성이 크지만 운영 영향이 큼 | 테스트 발행 전용 샌드박스 |
| 5 | 카카오톡/문자 | 알림 기능 확장 가능. 기존 알림톡 설정과 중복 검토 필요 | 템플릿/발신번호/전송결과 조회 중심 |
...
- [ ] 조회/전송 로그 테이블 또는 저장 컬렉션 설계
- [ ] 원문 응답 저장 여부 결정
- [ ] 사업자번호, 계좌번호, 전화번호 마스킹 저장 기준 결정
...
- [ ] 홈택스수집: 부서사용자 방식과 공동인증서 방식 중 우선 방식 결정
- [ ] 홈택스수집: 요청 기간 제한, JobID 유효 시간, 상태 조회 흐름 확인
- [ ] 전자세금계산서: 정발행/역발행/위수탁 중 필요한 발행 유형 결정
- [ ] 전자세금계산서: 테스트 환경에서 국세청 전송 가상화 범위 확인
- [ ] 카카오톡: 비즈니스 채널, 승인 템플릿, 대체문자 사용 여부 확인
- [ ] 문자: 발신번호 등록, SMS/LMS/MMS 제한, 이미지 규격 확인
