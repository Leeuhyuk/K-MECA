...
function _defaultDocTemplateBook(type) {
  ...
  // 비고 박스: 내부 가로줄 없이 바깥 테두리만 (PDF .remarks 와 동일)
  const _rmEdge = { style:'thin', color:{argb:'FFCCCCCC'} };
  for (let r = 34; r <= 37; r++) {
    for (let c = 1; c <= 12; c++) ws.getCell(r, c).border = {};   // 전체 초기화
    // HTML 렌더는 병합 마스터(A열)의 좌우 테두리로 박스 폭을 그린다
    ws.getCell(r, 1).border = { left:_rmEdge, right:_rmEdge,
      top:r===34?_rmEdge:undefined, bottom:r===37?_rmEdge:undefined };
    ws.getCell(r, 12).border = { right:_rmEdge,                   // 실제 엑셀 우측 끝
      top:r===34?_rmEdge:undefined, bottom:r===37?_rmEdge:undefined };
  }
  for (let c = 2; c <= 11; c++) {                                 // 실제 엑셀 상/하 외곽
    ws.getCell(34, c).border = { top:_rmEdge };
    ws.getCell(37, c).border = { bottom:_rmEdge };
  }
